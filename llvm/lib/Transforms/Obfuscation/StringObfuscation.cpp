...
STATISTIC(GlobalsEncoded, "String of global variables will be obfuscated");
...
public:
   ...
   virtual bool runOnModule(Module &M) override {
      ...
      OBFUS_INFO(("StringObfuscationPass::runOnModule : is_flag : %d", is_flag));
      ...
      for (Module::global_iterator gi = M.global_begin(), ge = M.global_end(); gi != ge; ++gi) {
         ...
         OBFUS_INFO(("1 [SECTION]:%s : | NAME:%s |", section.c_str(), gv->getName().str().c_str()));
//         errs() << "[SECTION] : " << section.c_str() << " NAME : " << "\"" << gv->getName().str() << "\"" << "\n";
         
         // Let's encode the static ones
         if (gv->isConstant()
             && gv->hasInitializer()
             && gv->getName().str().substr(0,4)==".str"
...
//               OBFUS_INFO(("    2 [SECTION]:%s | orig:%s", section.c_str(), orig));
               OBFUS_INFO(("    3 [SECTION] NAME:%s | orig:%s |", gv->getName().str().c_str(), orig));
